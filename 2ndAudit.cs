...
{
    public partial class _2ndAudit : Form
    {
        ...
        public static string checksheetno;
        string aaron;
        public bool RR;
        public _2ndAudit()
        {
        ...
        private void DisplayData()
        {
           ...
           // dataGridView1.Rows[Convert.ToInt16(Number)].Selected = true;
        ...
        private void _2ndAudit_Load(object sender, EventArgs e)
        {
            lblTypeAudit.Text = PSCategory.inline;
            TypeNG();
            InputBy();
        ...
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("select [2ndAudit] as one_start,[2ndEnd] as one_end,status2 from Temp_MasterAudit where AuditID = '" + Monitoring._instance.MySelectedAuditID + "' AND Process = '" + lblProcess.Text + "' AND ChecksheetNo = '" + lblCount.Text + "'", con);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
                string first_audit;
        ...
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("select Remarks2,Comment2 from Temp_MasterAudit where AuditID = '" + Monitoring._instance.MySelectedAuditID + "' AND Process = '" + lblProcess.Text + "' AND ChecksheetNo = '" + textBox7.Text + "'", con);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
                string comment2;
                string remarks2;
                comment2 = reader["Comment2"].ToString();
            ...
            try
            {
                ...
                if (txtStatus.Text != "" && lblFirstStart.Text != "" && lblFirstEnd.Text != "")
                {
                    ...
                    else
                    {
                        ...
                        cmd = new SqlCommand("update Temp_MasterAudit set Status2=@status2, Remarks2=@remarks2, Comment2=@comment2, [2ndAudit]=@2ndaudit, [2ndEnd]=@2ndend, Date=@date, Flag2=@flag2 where TypeofAudit = '" + lblTypeAudit.Text + "' AND AuditID= '" + Monitoring._instance.MySelectedAuditID + "' AND ChecksheetNo= '" + lblCount.Text + "' AND Process= '" + lblProcess.Text + "'", con);
                        con.Open();
                        cmd.Parameters.AddWithValue("@id", ID);
                        cmd.Parameters.AddWithValue("@status2", txtStatus.Text);
                        cmd.Parameters.AddWithValue("@remarks2", tbRemarks.Text);
                        cmd.Parameters.AddWithValue("@comment2", txtComment.Text);
                        cmd.Parameters.AddWithValue("@date", date1);
                        cmd.Parameters.AddWithValue("@2ndaudit", lblFirstStart.Text);
                        cmd.Parameters.AddWithValue("@2ndend", lblFirstEnd.Text);
        ...
        private void next_data()
        {
            ...
            con.Open();
            SqlDataAdapter sda = new SqlDataAdapter("SELECT COUNT(ID) FROM Temp_MasterAudit where AuditID = '" + Monitoring._instance.MySelectedAuditID + "' AND Process = '" + lblProcess.Text + "' AND ChecksheetNo = '" + textBox7.Text + "' AND status2!=''", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            con.Close();
        ...
        {
            con.Open();
            SqlCommand cmd = new SqlCommand("select [2ndAudit] as one_start,[2ndEnd] as one_end,status2 from Temp_MasterAudit where AuditID = '" + id + "' AND Process = '" + lblProcess.Text + "' AND ChecksheetNo = '" + lblCount.Text + "'", con);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
            {
                string first_audit;
        ...
        private void btnEnd_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand("update Temp_MasterAudit set Status2=@status2, Remarks2=@remarks2, Comment2=@comment2, [2ndAudit]=@2ndaudit, [2ndEnd]=@2ndend, Date=@date, Flag2=@flag2 where AuditID = '" + Monitoring._instance.MySelectedAuditID + "' AND ChecksheetNo= '" + lblCount.Text + "' AND Process= '" + lblProcess.Text + "'", con);
            con.Open();
            cmd.Parameters.AddWithValue("@id", ID);
            cmd.Parameters.AddWithValue("@status2", txtStatus.Text);
            cmd.Parameters.AddWithValue("@remarks2", tbRemarks.Text);
            ...
            if (RR == true)
            {
                ...
                lblTStart.Text = CRUD.dt.Rows[0]["2ndAudit"].ToString();
            ...
            {
                var withBlock = CRUD.dt.Rows[0];
                lblTEnd.Text = CRUD.dt.Rows[0]["2ndEnd"].ToString();
            ...
            if (lblCount.Text == lblChecksheetCountTotal.Text)
            {
                ...
                string select = "Select * from Temp_MasterAudit where Status2 = 'NG' AND TypeofAudit != 'PS Process Audit' AND AuditID = '" + Monitoring._instance.MySelectedAuditID + "'";
        ...
        private void btnDoneAudit_Click(object sender , EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
                    cmd = new SqlCommand("update Temp_MasterAudit set Status3=@status3, Status4=@status4, Status5=@status5, Status6=@status6, Status7=@status7, Remarks3=@remarks3, Remarks4=@remarks4, Remarks5=@remarks5, Remarks6=@remarks6, Remarks7=@remarks7, Comment3=@comment3, Comment4=@comment4, Comment5=@comment5, Comment6=@comment6, Comment7=@comment7, TypeNG3=@typeng3, TypeNG4=@typeng4, TypeNG5=@typeng5, TypeNG6=@typeng6, TypeNG7=@typeng7, NGDetails3=@ngdetails3, NGDetails4=@ngdetails4, NGDetails5=@ngdetails5, NGDetails6=@ngdetails6, NGDetails7=@ngdetails7, Date=@date, [3rdAudit]=@3audit, [4thAudit]=@4audit, [5thAudit]=@5audit, [6thAudit]=@6audit, [7thAudit]=@7audit, [3rdEnd]=@3end, [4thEnd]=@4end, [5thEnd]=@5end, [6thEnd]=@6end, [7thEnd]=@7end, Flag3=@flag3, Flag4=@flag4, Flag5=@flag5, Flag6=@flag6, Flag7=@flag7 where AuditID = '" + Monitoring._instance.MySelectedAuditID + "'", con);
                    con.Open();
                    cmd.Parameters.AddWithValue("@id", ID);
                    cmd.Parameters.AddWithValue("@status3", "N/A");
                    cmd.Parameters.AddWithValue("@status4", "N/A");
                    cmd.Parameters.AddWithValue("@st+atus5", "N/A");
                    ...
                    {
                        var withBlock = CRUD.dt.Rows[0];
                        lblTEnd1.Text = CRUD.dt.Rows[0]["2ndEnd"].ToString();
                    }
                }
                catch (Exception ex)
                {
        ...
        private void jeButton6_Click(object sender, EventArgs e)
        {
            ...
            cmd = new SqlCommand("update Temp_MasterAudit set TypeNG2= '" + cmbNG.Text + "', NGDetails2=@ngdetails2 where AuditID = '" + Monitoring._instance.MySelectedAuditID + "'  AND ChecksheetNo= '" + lblCount.Text + "' AND Process= '" + lblProcess.Text + "'", con);
            con.Open();
            cmd.Parameters.AddWithValue("@id", ID);
            cmd.Parameters.AddWithValue("@ngdetails2", textBox6.Text);
            cmd.ExecuteNonQuery();
            con.Close();
        ...
        private void CompPanel_MouseDown(object sender, MouseEventArgs e)
        {
