...
{
    public partial class Pending_Approval2 : Form
    {
        public static Pending_Approval2 _instance;
        ...
        string ID;
        public Pending_Approval2()
        {
            _instance = this;
            InitializeComponent();
        }

        private void Pending_Approval2_Load(object sender, EventArgs e)
        {
            InputBy();
        ...
        private void DisplayData()
        {
            ...
            adapt = new SqlDataAdapter("Select DISTINCT AuditID,Auditor,Date,Line,Process,Series,TypeofAudit,[View],[delete],[Change],Shift,Model,First_Approver,Second_Approver,Third_Approver from Approval where Second_Approver = '" + LOGIN.loginName + "' AND Status = 'Ongoing' AND  case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc", con);
            adapt.Fill(dt);
        ...
        private void DisplayData2()
        {
            con.Open();
            DataTable dt = new DataTable();
            adapt = new SqlDataAdapter("select DISTINCT AuditID,Auditor,Second_DateApproved,Line,Process,Series,TypeofAudit,[delete],Shift,Model from Approval where Second_Approver = '" + LOGIN.loginName + "' AND Second_Status = 'Done'", con);
            adapt.Fill(dt);
        ...
        private void Admin()
        {
            con.Open();
            DataTable dt = new DataTable();
            adapt = new SqlDataAdapter("select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],[Change],Model,Shift,First_Approver,Second_Approver,Third_Approver from Approval where Section = '" + LOGIN.LoginSection + "'  AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc", con);
            adapt.Fill(dt);
            dataGridView1.DataSource = dt;
            con.Close();
        ...
        public void searchACC()
        {
            ...
            if (cmbSearchBy.Text == "Audit ID")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],[Change],Model,Shift from Approval where AuditID like @auditid AND Second_Approver = '" + LOGIN.loginName + "' AND Section = '" + LOGIN.LoginSection + "'  AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
            ...
            else if (cmbSearchBy.Text == "Date")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],[Change],Model,Shift from Approval where Date like @date AND Second_Approver = '" + LOGIN.loginName + "' AND Section = '" + LOGIN.LoginSection + "' AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
            ...
            else if (cmbSearchBy.Text == "Line")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],[Change],Model,Shift from Approval where Line like @line AND Second_Approver = '" + LOGIN.loginName + "' AND Section = '" + LOGIN.LoginSection + "'  AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
            ...
            else if (cmbSearchBy.Text == "Series")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],[Change],Model,Shift from Approval where Series like @series AND Second_Approver = '" + LOGIN.loginName + "' AND Section = '" + LOGIN.LoginSection + "'  AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
            ...
            else if (cmbSearchBy.Text == "Model")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],[Change],Model,Shift from Approval where Model like @model AND Second_Approver = '" + LOGIN.loginName + "' AND Section = '" + LOGIN.LoginSection + "'  AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
        ...
        public void searchACC2()
        {
            ...
            if (cmbSearchBy.Text == "Audit ID")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],Shift,Model,First_Approver,Second_Approver,Third_Approver from Approval where AuditID like '%" + txtSearch.Text + "%' AND Second_Approver = '" + LOGIN.loginName + "'  AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
                    SqlDataAdapter sda = new SqlDataAdapter(cmd);
                    DataTable dt = new DataTable();
            ...
            else if (cmbSearchBy.Text == "Date")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],Shift,Model,First_Approver,Second_Approver,Third_Approver from Approval where Date like @date AND Second_Approver = '" + LOGIN.loginName + "'  AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.Add(new SqlParameter("@date", "%" + Date.Text + "%"));
                    SqlDataAdapter sda = new SqlDataAdapter(cmd);
            ...
            else if (cmbSearchBy.Text == "Line")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],Shift,Model,First_Approver,Second_Approver,Third_Approver from Approval where Line like @line AND Second_Approver = '" + LOGIN.loginName + "'  AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.Add(new SqlParameter("@line", "%" + txtSearch.Text + "%"));
                    SqlDataAdapter sda = new SqlDataAdapter(cmd);
            ...
            else if (cmbSearchBy.Text == "Series")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],Shift,Model,First_Approver,Second_Approver,Third_Approver from Approval where Series like @series AND Second_Approver = '" + LOGIN.loginName + "'  AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.Add(new SqlParameter("@series", "%" + txtSearch.Text + "%"));
                    SqlDataAdapter sda = new SqlDataAdapter(cmd);
            ...
            else if (cmbSearchBy.Text == "Model")
            {
                using (SqlConnection conn = new SqlConnection(BBB))
                {
                    string sql = "select DISTINCT AuditID,Auditor,Date,Line,Process,Series,[View],[delete],Shift,Model,First_Approver,Second_Approver,Third_Approver from Approval where Model like @model AND Second_Approver = '" + LOGIN.loginName + "' AND Status = 'Ongoing' AND case when Second_Status is NULL then 0 when Second_Status = '' then 0 else 1 end=0 order by Date asc";
                    SqlCommand cmd = new SqlCommand(sql, conn);
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.Add(new SqlParameter("@model", "%" + txtSearch.Text + "%"));
                    SqlDataAdapter sda = new SqlDataAdapter(cmd);
        ...
        private void dataGridView1_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (dataGridView1.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString() == "👁️")
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    ...
                    ApprovalLookup2 lookup = new ApprovalLookup2();
                    ...
                    lookup.lblAuditor.Text = this.dataGridView1.CurrentRow.Cells["Auditor"].Value.ToString();
        ...
        private void dataGridView2_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            ...
            dataGridView2.Columns["Second_DateApproved"].Width = 90;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (cmbChecker.Text != "")
            {
                cmd = new SqlCommand("update tbl_AuditApproval set Second_Approver=@secondapprover where AuditID=@auditid", con);
                con.Open();
                cmd.Parameters.AddWithValue("@auditid", lblAuditID.Text);
                cmd.Parameters.AddWithValue("@secondapprover", cmbChecker.Text);
                cmd.ExecuteNonQuery();
                con.Close();
